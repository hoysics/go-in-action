...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	...
	for _, s := range segs {
		...
		for _, child := range queue {
			childs = append(childs, child.childrenOf(s)...)
		}
		queue = childs
		wholeq = append(wholeq, queue...)
	}
	//再集中获取中间件
	for _, child := range wholeq {
		if child.mdls != nil && len(child.mdls) > 0 {
	...
	//	for 0 < counter {
	//		counter--
	//		if queue[0].paramChild != nil {
	//			queue = append(queue, queue[0].paramChild)
	//		}
	//		if queue[0].starChild != nil {
	//			queue = append(queue, queue[0].starChild)
	//		}
	//		queue = queue[1:]
	//	}
	//}
	//cur := root
	//for cur != nil {
	//	if cur.paramChild != nil && cur.paramChild.mdls != nil {
	//		mdls = append(mdls, cur.paramChild.mdls...)
	//	}
	//	if cur.starChild != nil && cur.starChild.mdls != nil {
