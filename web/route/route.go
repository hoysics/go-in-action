...
func (n *node) childOrCreate(path string) *node {
	...
	if path[0] == ':' {
		...
		if isReg {
			return n.childOrCreateReg(path, expr, param)
		}
...
func (n *node) childOrCreateParam(path, param string) *node {
	if n.starChild != nil {
	...
	if n.paramChild != nil {
		...
		n.paramChild = &node{path: path, paramName: param, typ: nodeTypeParam}
	}
	return n.paramChild
}

func (n *node) childOrCreateReg(path, expr, param string) *node {
	if n.starChild != nil {
		panic(fmt.Sprintf("web: 非法路由，已有通配符路由。不允许同时注册通配符路由和参数路由 [%s]", path))
	}
	if n.paramChild != nil {
	...
	if n.regChild != nil {
		if n.regChild.regExpr.String() != expr || n.regChild.paramName != param {
			panic(fmt.Sprintf("web: 路由冲突，正则路由冲突，已有 %s，新注册 %s", n.regChild.path, path))
		}
	} else {
		reg, err := regexp.Compile(expr)
		if err != nil {
			panic(fmt.Sprintf("web: 正则表达式解析错误: %v", err))
		}
		n.regChild = &node{
			typ:       nodeTypeReg,
			path:      path,
